...
const Gov = require("../model/govSchema");
router.get("/", (req, res) => {
      ...
      .exec(function (err, data) {
        if (err) {
          ...
          if (fvg === undefined) {
            ...
            const filteredUsers = mm.filter(user => {
              let isValid = true;
              for (key in filters) {
...
router.post("/commodity", async (req, res) => {
  const { commodity, min_price, max_price, modal_price } = req.body;
  //   let names= req.rootUser.name; 
  //  console.log(names)

  try {
    const gov = new Gov({ commodity, min_price, max_price, modal_price });
    const govData = await gov.save();
